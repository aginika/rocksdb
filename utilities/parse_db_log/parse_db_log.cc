...
namespace rocksdb
{
  ...
  bool TableEvent::Parse(std::string log_string)
  {
    std::sscanf(log_string.c_str(),
		"[%s] Generated table #%ld: %d keys, %ld bytes",
		name_,
		&id_,
		&key_num_,
		&bytes_
...
namespace rocksdb
{
  void LOGParser::Parse(std::string filename)
  {
	...
	if(ret != 0){
	  ...
	  if ( log_type == kCompactionEvent){
	    ...
	    if(log_string.find("Table was ") == std::string::npos
	       && log_string.find("because we have ") == std::string::npos
	       && log_string.find("summary") == std::string::npos){
	      std::cout << "NoUseEvent " << log_string << std::endl;
	    }
	  }
	}
	counter++;
  ...
  EventLogType LOGParser::GetTypeOfToken(std::string token){
    if (token.find(" Compact") != std::string::npos)
      ...
      return kFlushEvent;
    else if(token.find("Generated table") != std::string::npos)
      return kTableEvent;
